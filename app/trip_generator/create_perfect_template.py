...
def create_perfect_template():
    ...
    # 设置列宽
    table.columns[0].width = Inches(0.4)   # Day - 最短，只放数字
    table.columns[1].width = Inches(1.0)   # Date  
    table.columns[2].width = Inches(1.2)   # City
    table.columns[3].width = Inches(3.0)   # Touring spots - 更宽
    table.columns[4].width = Inches(2.8)   # Accommodation - 更宽
    ...
    for i, sample_text in enumerate(data_values):
        ...
        else:  # Day, Date, City, Touring spots 都居中
            para.alignment = WD_ALIGN_PARAGRAPH.CENTER
    ...
    print("✅ 创建了完美的模板: template_perfect.docx")
