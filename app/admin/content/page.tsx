"use client"

import dynamic from "next/dynamic"

const AdminContentClientPage = dynamic(() => import("./AdminContentClientPage"), {
  ssr: false,
  loading: () => (
    <div className="space-y-6">
      <AdminPageHeaderSkeleton />
      <div className="h-80 animate-pulse rounded-xl border bg-white dark:bg-gray-950" />
      <div className="h-72 animate-pulse rounded-xl border bg-white dark:bg-gray-950" />
    </div>
  ),
})

function AdminPageHeaderSkeleton() {
  return (
    <div className="flex items-center justify-between">
      <div className="space-y-2">
        <div className="h-9 w-40 animate-pulse rounded-lg bg-gray-200 dark:bg-gray-800" />
        <div className="h-5 w-56 animate-pulse rounded bg-gray-200 dark:bg-gray-800" />
      </div>
      <div className="h-10 w-24 animate-pulse rounded-md bg-gray-200 dark:bg-gray-800" />
    </div>
  )
}

export default function AdminContentPage() {
  return <AdminContentClientPage />
}
