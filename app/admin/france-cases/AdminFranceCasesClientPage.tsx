...
import { FRANCE_EXCEPTION_CODES, FRANCE_MAIN_STATUSES } from "@/lib/france-case-machine"
...
export default function AdminFranceCasesClientPage() {
  ...
  const fetchData = useCallback(async (showRefresh = false) => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/france-cases?${params.toString()}`, {
        cache: "no-store",
      })
      const data = await response.json()
      if (!response.ok || !data.success) {
        throw new Error(data?.message || "加载法签案件后台失败")
      }
      ...
      console.error("加载法签案件后台失败:", error)
      setBannerMessage(error instanceof Error ? error.message : "加载法签案件后台失败")
    } finally {
      setLoading(false)
      setRefreshing(false)
    }
  ...
  const updateReminderStatus = async (logId: string, sendStatus: string) => {
    ...
    try {
      ...
      const response = await fetch("/api/admin/france-cases", {
        method: "PATCH",
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">法签案件与提醒</h1>
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            查看法签案件进度、异常状态，以及 ReminderLog 触发后的待处理提醒。
          </p>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
            ...
            <Select value={mainStatus} onValueChange={setMainStatus}>
              ...
              <SelectContent>
                ...
                {FRANCE_MAIN_STATUSES.map((status) => (
                  <SelectItem key={status} value={status}>
                    {formatFranceStatusLabel(status, null)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <Select value={exceptionCode} onValueChange={setExceptionCode}>
              ...
              <SelectContent>
                ...
                {FRANCE_EXCEPTION_CODES.map((item) => (
                  <SelectItem key={item} value={item}>
                    {formatFranceExceptionLabel(item)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
              ...
              <TableBody>
                {cases.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{formatFranceStatusLabel(item.mainStatus, item.subStatus)}</TableCell>
                    <TableCell>
                      {item.exceptionCode ? (
                        <Badge variant="destructive">{formatFranceExceptionLabel(item.exceptionCode)}</Badge>
                      ) : (
              ...
              <TableBody>
                {reminderLogs.map((log) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>{formatFranceStatusLabel(log.visaCase.mainStatus, log.visaCase.subStatus)}</TableCell>
