...
import {
  getUsaReminderAdminSummary,
  listUsaCasesForAdmin,
...
export async function GET(request: NextRequest) {
  if (isPublicUiPreviewEnabled()) {
    return NextResponse.json(getPublicUiPreviewAdminData("usa-cases"))
  }
  ...
  try {
    ...
    console.error("获取美签案件后台数据失败:", error)
    return NextResponse.json(
      {
        success: false,
        message: "获取美签案件后台数据失败",
      },
      { status: 500 },
    )
  }
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const reminderLog = await updateUsaReminderLogStatus({
      logId,
      sendStatus,
      errorMessage,
      renderedContent,
