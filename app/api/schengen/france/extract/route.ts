  ...
  try {
    ...
    if (files.length === 0) {
      return NextResponse.json(
        { success: false, error: "请上传至少一个 Excel 文件（含 FV注册表）" },
        { status: 400 }
      )
    }
    ...
    try {
      ...
      return NextResponse.json(
        { success: false, error: "法签提取服务未找到，请确认 services/french-visa 已部署" },
        { status: 500 }
      )
    }
    ...
    const proc = spawn(
      "python",
      ["-u", scriptPath, ...inputPaths, "--output-dir", outputDir],
      { cwd: process.cwd(), env: { ...process.env, PYTHONIOENCODING: "utf-8", PYTHONUTF8: "1" } }
    )
    ...
    return NextResponse.json({
      ...
      message: `已创建提取任务，请在下方的任务列表中查看进度`,
    })
  } catch (error) {
