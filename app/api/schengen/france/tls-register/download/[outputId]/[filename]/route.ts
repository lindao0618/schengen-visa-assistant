...
async function resolveFilePath(outputId: string, safeName: string) {
  const baseDir = path.join(process.cwd(), "temp", "french-visa-tls-register", outputId)
...
export async function GET(
  ...
  try {
    ...
    const canAccess = await canAccessFrenchVisaTaskOutput(session.user.id, outputId, "fv-tls-register-")
    if (!canAccess) {
    ...
    console.error("[tls-register download]", e)
