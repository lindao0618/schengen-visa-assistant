...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const file of files) {
      ...
      const task = await createTask(session.user.id, "create-application", `生成新申请 · ${fileName}`)
      ...
      proc.on("close", async (code) => {
        ...
        try {
          ...
          if (data.success && (data.excel_file || data.json_file)) {
            const download_excel = data.excel_file
              ? `/api/schengen/france/create-application/download/${outputId}/${encodeURIComponent(data.excel_file)}`
              : undefined
